...
function createUpdatedCollection(collectionA, objectB) {
  ...
  for(var i = 0; i<collectionA.length; i++){
    ...
    if(!exist){
      if(!collectionA[i].includes("-")){
        ...
        var key = collectionA[i].split('-')[0];
        var qty = collectionA[i].split('-')[1];
        collectionA_Arr.push({key : key, count : qty});
      }
    }
  }
  ...
  //     var key = collectionA[i].split('-')[0];
  //     var qty = collectionA[i].split('-')[1];
  //     for(var j = 0; j<collectionA_Arr.value.length; j++){
  //       if(collectionA_Arr[j].key === key){
  //         collectionA_Arr[j].count -= qty;
  //       }
  //   }
  // }
